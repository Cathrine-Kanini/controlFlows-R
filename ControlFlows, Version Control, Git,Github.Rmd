...
**Architecture:**
- Single central server stores all files
- Developers check out files from central location
- Changes committed directly to central server

**Workflow:**
1. **Checkout/Update**: Get latest files from server
2. **Make Changes**: Edit files locally
3. **Commit**: Save changes to central server
...
- Changes shared through push/pull

**Workflow:**
1. **Commit**: Save changes to local repository
2. **Push**: Upload changes to remote repository
3. **Pull/Fetch**: Download changes from remote repository
...
- Takes a score input
- Uses if-else-if to assign letter grade
- Uses switch to assign GPA
- Prints formatted result

## Exercise 2: Loops

Write R code to:
- Calculate Fibonacci sequence (for loop)
- Find prime numbers (while loop)
- Create menu system (repeat loop)
...
- [R Programming Cheat Sheet](https://cran.r-project.org/doc/contrib/Short-refcard.pdf)
